...
import { X, User, Search, UserPlus, Loader2 } from 'lucide-react'
...
interface Props {
  ...
  onClose: () => void
  onAdd: (profile: CandidateProfile) => Promise<void>
}

export default function AddTeacherModal({ candidates, onClose, onAdd }: Props) {
  ...
  const handleClick = async (c: CandidateProfile) => {
    setAdding(c.id)
    setError(null)
    try {
      await onAdd(c)
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end md:items-center justify-center p-0 md:p-4">
      <div className="bg-white w-full md:max-w-md md:rounded-2xl rounded-t-2xl max-h-[85vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-gray-100">
          <div>
            ...
            <p className="text-[11px] text-cyan-600 mt-1">
              หลังเพิ่มแล้ว ครูจะ login ที่เครื่องตัวเองเพื่อสแกนใบหน้า
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {error && (
          <p className="mx-4 mt-3 text-sm text-red-600 text-center bg-red-50 px-3 py-2 rounded-xl">
            {error}
          </p>
        )}
            ...
            filtered.map((c) => {
              ...
              return (
                <button
                  ...
                  disabled={!!adding}
                  className="w-full flex items-center gap-3 p-3 bg-gray-50 hover:bg-cyan-50 disabled:opacity-50 rounded-xl transition-colors text-left"
                >
                  ...
                  {busy ? (
                    ...
                    <UserPlus className="w-4 h-4 text-cyan-500 shrink-0" />
                  )}
                </button>
              )
            })
